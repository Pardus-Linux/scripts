...
COMMENT_BUG_ACTIVITY_STATUS = """INSERT INTO bugs_activity (bug_id, who, bug_when, fieldid, added, removed) VALUES ('%(bug_id)s', %(user_id)d, '%(bug_when)s', %(fieldid)d, '%(added)s', '%(removed)s')"""
COMMENT_BUG_ACTIVITY_RESOLUTION = """INSERT INTO bugs_activity (bug_id, who, bug_when, fieldid, added, removed) VALUES ('%(bug_id)s', %(user_id)d, '%(bug_when)s', %(fieldid)d, '%(added)s', '%(removed)s')"""
...
STATUS_SQL = "SELECT bug_status FROM `bugs` WHERE bug_id='%(bug_id)s'"
RESOLUTION_SQL = "SELECT resolution FROM `bugs` WHERE bug_id='%(bug_id)s'"
...
def main(author, log, commit_no, changed, repo):
    ...
    def getOldBugStatus():
        if cur.execute(STATUS_SQL % {"bug_id": bug_id}) == 1:
            return cur.fetchone()[0]
        return 1

    def getOldResolution():
         if cur.execute(RESOLUTION_SQL % {"bug_id": bug_id}) == 1:
             return cur.fetchone()[0]
         return 1
    ...
    def commentBUG(bug_id):
        ...
        fieldid = 8
        added = "RESOLVED"
        cur.execute(COMMENT_BUG_ACTIVITY_STATUS % {"bug_id": bug_id, "user_id": getAuthorBugzillaID(), "bug_when": cur_time, "fieldid" : fieldid, "added" : added, "removed": getOldBugStatus()})
        db.commit()

        fieldid = 11
        added = "FIXED"
        cur.execute(COMMENT_BUG_ACTIVITY_RESOLUTION % {"bug_id": bug_id, "user_id": getAuthorBugzillaID(), "bug_when": cur_time, "fieldid" : fieldid, "added" : added, "removed": getOldBugResolution()})
        db.commit()
