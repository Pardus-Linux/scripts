...
class Pspec:
	def __init__(self, pspecfile):
		try:
			...
			print 'Unable to parse %s file' % pspecfile
			sys.exit(1)
...
class KdeLanguageUpdater:
	...
	pspec_header = '''<?xml version="1.0" encoding="utf-8" standalone="no"?>
...
'''
	...
	def updatePspec(self, pspec_filename, version, comment, name, email):
		...
		try:
			...
			print 'Kaynak %s ismi güncelleme listesinde yok' % (source_name)
			return
		...
		print '%s kaynak dosyası güncellendi.' % source_name

if __name__ == '__main__':
	print 'Kde i18n Pisi Package Updater v0.1'
	
	if len(sys.argv) != 7:
		print 'Usage: i18nup [path] [upfile] [version] [comment] [developer name] [developer mail]'
		print 'Example: i18nup /home/furkan/svn/devel/i18n/ 3.4.3 "Version bump" "Furkan Duman" "coderlord at gmail.com"'
		sys.exit()
